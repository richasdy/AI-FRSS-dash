<script lang="ts">
	...
	import GridShape from '../../../components/gridshape/GridShape.svelte';
	...
	const { form, errors, isSubmitting, reset } = createForm<SignUpPayload>({
						...
						<form use:form>
							<div class="space-y-5">
								...
								<div>
									...
									/>
									{#if $errors.name}<p class="text-error-500 mt-2 text-sm">
											{$errors.name[0]}
										</p>{/if}
								</div>
								<!-- Email -->
								<div>
									...
									<input
										...
										class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
									/>
									{#if $errors.email}<p class="text-error-500 mt-2 text-sm">
											{$errors.email[0]}
										</p>{/if}
								</div>
								<!-- Password -->
								<div>
									...
									<div class="relative">
										...
										<button
											...
											class="absolute top-1/2 right-4 z-30 m-0 -translate-y-1/2 cursor-pointer border-0 bg-transparent p-0 text-gray-500 dark:text-gray-400"
											tabindex="0"
									...
									{#if $errors.password}<p class="text-error-500 mt-2 text-sm">
											{$errors.password[0]}
										</p>{/if}
								</div>
								<!-- Button -->
			...
			<div class="z-1 flex items-center justify-center">
				...
				<div class="flex max-w-xs flex-col items-center">
					<a href="index.html" class="mb-4 block">
						<img src="/logo/auth-logo.svg" alt="Logo" />
					</a>
					<p class="text-center text-gray-400 dark:text-white/60">
						Svelte Frontend App with TailwindCSS
					</p>
				</div>
			</div>
		</div>
