<script lang="ts">
  ...
  let selectedApprovalFilter: 'Approved' | 'Pending' | '' = '';
  
  // --- Query untuk data yang DIFILTER (untuk tabel) ---
  // Define the queryKey for filtered users as a reactive variable
  $: filteredUsersQueryKey = ['filteredUsers', searchUsers, selectedRoleFilter, selectedStatusFilter, selectedApprovalFilter];
  const filteredUsersQuery = createQuery({
    queryKey: filteredUsersQueryKey, 
    queryFn: async () => {
      return await getAllUsers(searchUsers, selectedRoleFilter, selectedStatusFilter, selectedApprovalFilter);
    }
  });

  // --- Query untuk data SEMUA pengguna (untuk statistik) ---
  const allUsersQuery = createQuery({
    queryKey: ['allUsers'], 
    queryFn: async () => {
  ...
  // Query untuk mengambil data peran (tidak berubah)
  const rolesQuery = createQuery({
    queryKey: ['roles'],
    queryFn: () => getAllRoles()
  });

  // Reactive statements untuk menghitung statistik
  $: totalUsers = $allUsersQuery.data?.length || 0;
  $: activeNow = $allUsersQuery.data?.filter(user => user.isOnline).length || 0;
  $: administrators = $allUsersQuery.data?.filter(user => user.role?.name === 'Admin').length || 0;
  $: pendingApproval = $allUsersQuery.data?.filter(user => !user.isApproved).length || 0;

  // Fungsi untuk menghitung jumlah user per role
  $: roleUserCounts = $allUsersQuery.data?.reduce((acc, user) => {
    if (user.role?.name) {
  ...
  async function applyFilters() {
    ...
    await queryClient.invalidateQueries({ queryKey: filteredUsersQueryKey });
  }
  ...
  async function handleUserAction(action: 'approve' | 'reject' | 'delete', userId: string) {
    try {
      if (action === 'approve') {
    ...
    >
      <div class="flex items-center gap-x-4">
        ...
        <div class="flex flex-col">
          ...
          <p class="text-theme-sm text-gray-500 dark:text-gray-700">Total Users</p>
        </div>
      </div>
    </div>
    <div
    ...
    >
      <div class="flex items-center gap-x-4">
        ...
        <div class="flex flex-col">
          ...
          <p class="text-theme-sm text-gray-500 dark:text-gray-700">Active Now</p>
        </div>
      </div>
    </div>
    <div
    ...
    >
      <div class="flex items-center gap-x-4">
        ...
        <div class="flex flex-col">
          ...
          <p class="text-theme-sm text-gray-500 dark:text-gray-700">Administrator</p>
        </div>
      </div>
    </div>
    <div
    ...
    >
      <div class="flex items-center gap-x-4">
        ...
        <div class="flex flex-col">
          ...
          <p class="text-theme-sm text-gray-500 dark:text-gray-700">Pending Approval</p>
        </div>
      </div>
    </div>
  </div>
  ...
  >
    ...
    <div class="flex flex-col gap-y-4 px-6 py-5 lg:gap-y-6">
      ...
      <div class="max-w-full overflow-x-auto">
        {#key JSON.stringify(filteredUsersQueryKey)}
