"""
Universal YOLO API Template - SECURITY THREATS MODEL
...
MODEL_TYPE = "security_threats"

router = APIRouter()

@router.post("/detect", response_model=DetectionResponse)
async def detect_security_threats(request: DetectionRequest):
    """
    Detect security threats in image
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Security threats detection failed: {str(e)}")

@router.get("/info")
async def get_security_model_info():
    """
    Get security threats model information
...
@router.get("/history")
async def get_security_history(limit: int = 100):
    """
    Get security threats detection history
...
@router.post("/load")
async def load_security_model():
    """
    Load security threats model into memory
